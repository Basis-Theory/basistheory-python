...
from basistheory.exceptions import (
    ApiKeyError,
    ApiAttributeError,
    ApiTypeError,
    ApiValueError,
...
def check_validations(
    ...
    if (is_json_validation_enabled('maxLength', configuration) and
        ...
        try:
            if (len(input_values) > current_validations['max_length']):
                raise ApiValueError(
    ...
    if (is_json_validation_enabled('minLength', configuration) and
        ...
        try:
            if (len(input_values) < current_validations['min_length']):
                raise ApiValueError(
...
def model_to_dict(model_instance, serialize=True):
    ...
    def extract_item(item): return (item[0], model_to_dict(
        item[1], serialize=serialize)) if hasattr(item[1], '_data_store') else item
...
def set_request_options(request_options, self):
    config = self.api_client.configuration
    if request_options.api_key:
        config.api_key = {'apiKey': request_options.api_key}
